...
- 带 skill gating 的 workspace automations：simple watcher -> owner task -> report -> policy loop
...
watcher 只负责检测。当 watcher 上报 `needs_agent` 时，Sesame 会暂停该 watcher 的本轮调度，为这次信号只派发一个 owner task，等待 task 结果，然后根据 automation policy 决定恢复、暂停或升级。
...
修改 Web Console 行为时运行 console 测试：
