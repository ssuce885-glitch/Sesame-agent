...
func TestManagerStopsRunningShellTask(t *testing.T) {
	...
	task, err := manager.Create(context.Background(), CreateTaskInput{
		Type:          TaskTypeShell,
		Command:       longRunningTaskCommand(),
		WorkspaceRoot: root,
		Start:         true,
	})
	if err != nil {
...
func TestManagerRunsAgentTaskThroughExecutor(t *testing.T) {
	...
	executor := &fakeAgentExecutor{
		output:    "agent:summarize the workspace",
		finalText: "final workspace summary",
	}
	...
	result, ready, err := manager.ReadResult(task.ID, root)
	if err != nil {
		t.Fatalf("ReadResult() error = %v", err)
	}
	if !ready {
		t.Fatal("ReadResult() ready = false, want true")
	}
	if result.Kind != FinalResultKindAssistantText {
		t.Fatalf("ReadResult() kind = %q, want %q", result.Kind, FinalResultKindAssistantText)
	}
	if result.Text != "final workspace summary" {
		t.Fatalf("ReadResult() text = %q, want %q", result.Text, "final workspace summary")
	}
	if !reflect.DeepEqual(executor.gotActivatedSkillNames, task.ActivatedSkillNames) {
		t.Fatalf("activatedSkillNames = %v, want %v", executor.gotActivatedSkillNames, task.ActivatedSkillNames)
	}
}

func TestManagerMarksCompletionNotifiedAfterTerminalNotifierSucceeds(t *testing.T) {
	root := t.TempDir()
	notifier := &fakeTerminalNotifier{done: make(chan Task, 1)}
	manager := NewManager(Config{
		MaxConcurrentTasks: 8,
		TaskOutputMaxBytes: 1 << 20,
		TerminalNotifier:   notifier,
	}, nil, &fakeAgentExecutor{
	...
	waitForTaskTerminal(t, manager, created.ID, root)
	select {
	case notified := <-notifier.done:
		if notified.ID != created.ID {
			t.Fatalf("notified task id = %q, want %q", notified.ID, created.ID)
		}
		if !notified.ResultReady() {
			t.Fatalf("notified task result ready = false, want true")
		}
	case <-time.After(2 * time.Second):
		t.Fatal("timed out waiting for terminal notifier")
	}

	got, ok, err := manager.Get(created.ID, root)
	for attempt := 0; attempt < 20; attempt++ {
		...
		if got.CompletionNotifiedAt != nil {
			return
		}
		time.Sleep(10 * time.Millisecond)
	}
	t.Fatal("CompletionNotifiedAt = nil, want timestamp after notifier succeeds")
}

func TestManagerRunsRemoteTaskThroughShim(t *testing.T) {
	root := t.TempDir()
	shim, script := remoteShimFixture(root)
	mode := os.FileMode(0o644)
	if runtime.GOOS != "windows" {
		mode = 0o755
	}
	if err := os.WriteFile(shim, []byte(script), mode); err != nil {
		t.Fatal(err)
	}
...
func TestManagerWaitReturnsWhenTaskBecomesTerminal(t *testing.T) {
	root := t.TempDir()
	started := make(chan struct{}, 1)
	release := make(chan struct{})
	manager := NewManager(
		Config{MaxConcurrentTasks: 8, TaskOutputMaxBytes: 1 << 20},
		map[TaskType]Runner{
			TaskTypeShell: blockingRunner{started: started, release: release},
		},
		nil,
	)
	...
	go func() {
		...
		defer cancel()
		got, timedOut, err := manager.Wait(waitCtx, task.ID, root)
		resultCh <- struct {
			task     Task
			timedOut bool
			err      error
		}{task: got, timedOut: timedOut, err: err}
	}()

	select {
	case result := <-resultCh:
		t.Fatalf("Wait() returned early with %#v", result)
	...
	case result := <-resultCh:
		...
		if result.timedOut {
			t.Fatal("Wait() timedOut = true, want false")
		}
		if result.task.Status != TaskStatusCompleted {
			t.Fatalf("Wait() status = %q, want %q", result.task.Status, TaskStatusCompleted)
		}
	case <-time.After(time.Second):
...
func TestManagerWaitTimesOutAndReturnsCurrentSnapshot(t *testing.T) {
	root := t.TempDir()
	started := make(chan struct{}, 1)
	manager := NewManager(
		Config{MaxConcurrentTasks: 8, TaskOutputMaxBytes: 1 << 20},
		map[TaskType]Runner{
			TaskTypeShell: blockingRunner{started: started, release: make(chan struct{})},
		},
		nil,
	)
	...
	if !timedOut {
		t.Fatal("Wait() timedOut = false, want true")
	}
	if got.Status != TaskStatusRunning {
		t.Fatalf("Wait() status = %q, want %q", got.Status, TaskStatusRunning)
	}
...
func longRunningTaskCommand() string {
	if runtime.GOOS == "windows" {
...
func remoteShimFixture(root string) (string, string) {
	if runtime.GOOS == "windows" {
		return filepath.Join(root, "remote-shim.cmd"), "@echo off\r\necho remote:%~1\r\n"
	}
	return filepath.Join(root, "remote-shim.sh"), "#!/bin/sh\nprintf 'remote:%s\\n' \"$1\"\n"
}
	...
	t.Run("tasks round trip through write and load", func(t *testing.T) {
		...
		tasks := []Task{
			...
			},
			{
				...
				FinalResultText:    "workspace summary",
				FinalResultReadyAt: &end,
			},
		}
...
func TestAgentRunnerRun(t *testing.T) {
	t.Run("forwards workspace, prompt, and output", func(t *testing.T) {
		...
		if got := sink.finalTextFor(task.ID); got != executor.finalText {
			t.Fatalf("sink final text = %q, want %q", got, executor.finalText)
		}
	})
	...
	t.Run("requires final result sink support", func(t *testing.T) {
		runner := NewAgentRunner(&fakeAgentExecutor{})
		err := runner.Run(context.Background(), &Task{ID: "task_missing_sink"}, outputOnlySink{})
		if !errors.Is(err, errAgentFinalResultSinkNotConfigured) {
			t.Fatalf("Run() error = %v, want %v", err, errAgentFinalResultSinkNotConfigured)
		}
	})
}
...
func (f *fakeAgentExecutor) RunTask(ctx context.Context, workspaceRoot string, prompt string, activatedSkillNames []string, observer AgentTaskObserver) error {
...
func (f *fakeTerminalNotifier) NotifyTaskTerminal(_ context.Context, task Task) error {
	if f.done != nil {
		f.done <- task
	}
	return f.err
...
func (f *fakeOutputSink) SetFinalText(taskID, text string) error {
	if f.finalTexts == nil {
		f.finalTexts = make(map[string]string)
	}
	f.finalTexts[taskID] = text
	return nil
}

func (f *fakeOutputSink) finalTextFor(taskID string) string {
	if f.finalTexts == nil {
		return ""
	}
	return f.finalTexts[taskID]
}

type outputOnlySink struct{}

func (outputOnlySink) Append(string, []byte) error { return nil }
...
func (r blockingRunner) Run(ctx context.Context, _ *Task, _ OutputSink) error {
	if r.started != nil {
		select {
