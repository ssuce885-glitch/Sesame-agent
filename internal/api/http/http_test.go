...
type fakeStore struct {
	...
	called                    bool
	markInterruptedTurnID     string
	appendedEvents            []types.Event
}
...
func (s *fakeStore) MarkTurnInterrupted(ctx context.Context, turnID string) error {
	s.markInterruptedTurnID = turnID
	return nil
}

func (s *fakeStore) AppendEvent(ctx context.Context, event types.Event) (int64, error) {
	event.Seq = int64(len(s.appendedEvents) + 1)
	s.appendedEvents = append(s.appendedEvents, event)
...
func (m *fakeManager) ResumeTurn(ctx context.Context, sessionID string, in session.ResumeTurnInput) (string, error) {
	return in.TurnID, nil
}

func (m *fakeManager) InterruptTurn(sessionID, turnID string) bool {
	m.interruptedSessionID = sessionID
	m.interruptedTurnID = turnID
	return m.interruptShouldSucceed
}
...
func TestInterruptTurnMarksActiveSessionTurnInterrupted(t *testing.T) {
	store := &fakeStore{
		sessions: []types.Session{{
			ID:           "sess_1",
			ActiveTurnID: "turn_1",
	...
	manager := &fakeManager{interruptShouldSucceed: true}
	handler := NewRouter(Dependencies{
		Store:   store,
		Manager: manager,
	})

	req := httptest.NewRequest(http.MethodPost, "/v1/sessions/sess_1/interrupt", nil)
	...
	if manager.interruptedSessionID != "sess_1" {
		t.Fatalf("interrupted session = %q, want sess_1", manager.interruptedSessionID)
	}
	if manager.interruptedTurnID != "turn_1" {
		t.Fatalf("interrupted turn = %q, want turn_1", manager.interruptedTurnID)
	}
	if store.markInterruptedTurnID != "turn_1" {
		t.Fatalf("markInterruptedTurnID = %q, want turn_1", store.markInterruptedTurnID)
	}
	if len(store.appendedEvents) != 1 {
		t.Fatalf("appended events = %d, want 1", len(store.appendedEvents))
	}
	if store.appendedEvents[0].Type != types.EventTurnInterrupted {
		t.Fatalf("event type = %q, want %q", store.appendedEvents[0].Type, types.EventTurnInterrupted)
	}
}
...
func TestTimelineEndpointReturnsNormalizedBlocksAndLatestSeq(t *testing.T) {
	...
	if !strings.Contains(body, "\"turn_id\":\"turn_1\"") {
		t.Fatalf("body = %s, want turn_id preserved on timeline blocks", body)
	}
...
func TestTimelineEndpointIncludesSessionMemoryFailureNotice(t *testing.T) {
	deps := NewTestDependencies(t)
	store := deps.Store.(*sqlite.Store)
	now := time.Now().UTC()
	if err := store.InsertSession(context.Background(), types.Session{
		ID:            "sess_timeline_memory_fail",
		WorkspaceRoot: "E:/project/go-agent",
		State:         types.SessionStateIdle,
		CreatedAt:     now,
		UpdatedAt:     now,
	...
	event, err := types.NewEvent("sess_timeline_memory_fail", "turn_1", types.EventSessionMemoryFailed, types.SessionMemoryEventPayload{
	...
	handler := NewRouter(deps)
	req := httptest.NewRequest(http.MethodGet, "/v1/sessions/sess_timeline_memory_fail/timeline", nil)
	rec := httptest.NewRecorder()
	handler.ServeHTTP(rec, req)
	...
	body := rec.Body.String()
	if !strings.Contains(body, "会话记忆刷新失败：compact failed") {
		t.Fatalf("body = %s, want session memory failure notice", body)
	}
	if !strings.Contains(body, "\"turn_id\":\"turn_1\"") {
		t.Fatalf("body = %s, want failure notice turn_id", body)
	}
}

func TestTimelineEndpointIncludesTaskResultReadyBlock(t *testing.T) {
	deps := NewTestDependencies(t)
	store := deps.Store.(*sqlite.Store)
	now := time.Now().UTC()
	if err := store.InsertSession(context.Background(), types.Session{
		ID:            "sess_timeline_task_result",
		WorkspaceRoot: "E:/project/go-agent",
		State:         types.SessionStateIdle,
		CreatedAt:     now,
		UpdatedAt:     now,
	...
	event, err := types.NewEvent("sess_timeline_task_result", "turn_1", types.EventTaskResultReady, types.TimelineBlock{
		ID:            "task_child_1",
		TurnID:        "turn_1",
		Kind:          "task_block",
		Status:        "completed",
		...
		TaskID:        "task_child_1",
		ResultPreview: "child agent final answer",
	})
	if err != nil {
		t.Fatalf("NewEvent() error = %v", err)
	}
	...
	handler := NewRouter(deps)
	req := httptest.NewRequest(http.MethodGet, "/v1/sessions/sess_timeline_task_result/timeline", nil)
	rec := httptest.NewRecorder()
	handler.ServeHTTP(rec, req)
	...
	body := rec.Body.String()
	if !strings.Contains(body, "\"task_id\":\"task_child_1\"") {
		t.Fatalf("body = %s, want task block id", body)
	}
	if !strings.Contains(body, "\"result_preview\":\"child agent final answer\"") {
		t.Fatalf("body = %s, want task result preview", body)
	}
}

func TestTimelineEndpointIncludesPendingReportCount(t *testing.T) {
	deps := NewTestDependencies(t)
	store := deps.Store.(*sqlite.Store)
	now := time.Now().UTC()
	if err := store.InsertSession(context.Background(), types.Session{
		ID:            "sess_timeline_report_count",
		WorkspaceRoot: "E:/project/go-agent",
		State:         types.SessionStateIdle,
		CreatedAt:     now,
		UpdatedAt:     now,
	...
	if err := store.UpsertReportMailboxItem(context.Background(), types.ReportMailboxItem{
		ID:         "task_result:task_report_1",
		SessionID:  "sess_timeline_report_count",
	...
	handler := NewRouter(deps)
	req := httptest.NewRequest(http.MethodGet, "/v1/sessions/sess_timeline_report_count/timeline", nil)
	rec := httptest.NewRecorder()
	handler.ServeHTTP(rec, req)
	...
	body := rec.Body.String()
	if !strings.Contains(body, "\"pending_report_count\":1") {
		t.Fatalf("body = %s, want pending_report_count 1", body)
	}
}

func TestMailboxEndpointReturnsItemsAndPendingCount(t *testing.T) {
	deps := NewTestDependencies(t)
	store := deps.Store.(*sqlite.Store)
	now := time.Now().UTC()
	if err := store.InsertSession(context.Background(), types.Session{
	...
	handler := NewRouter(deps)
	req := httptest.NewRequest(http.MethodGet, "/v1/sessions/sess_mailbox/mailbox", nil)
	rec := httptest.NewRecorder()
	handler.ServeHTTP(rec, req)
	...
	body := rec.Body.String()
	if !strings.Contains(body, "\"pending_count\":1") {
		t.Fatalf("body = %s, want pending_count 1", body)
	}
	if !strings.Contains(body, "\"title\":\"Shanghai weather\"") {
		t.Fatalf("body = %s, want mailbox item title", body)
	}
}

func TestCronEndpointsListAndMutateJobs(t *testing.T) {
	deps := NewTestDependencies(t)
	store := deps.Store.(*sqlite.Store)
	service := scheduler.NewService(store, nil)
	deps.Scheduler = service
	handler := NewRouter(deps)

	jobA, err := service.CreateJob(context.Background(), scheduler.CreateJobInput{
		Name:           "Shanghai weather",
		WorkspaceRoot:  "/tmp/workspace-a",
		OwnerSessionID: "sess_cron_a",
		Prompt:         "每五分钟汇报上海天气",
		EveryMinutes:   5,
	})
	if err != nil {
	...
	_, err = service.CreateJob(context.Background(), scheduler.CreateJobInput{
		Name:           "Hangzhou weather",
		WorkspaceRoot:  "/tmp/workspace-b",
		OwnerSessionID: "sess_cron_b",
		Prompt:         "每五分钟汇报杭州天气",
		EveryMinutes:   5,
	})
	if err != nil {
	...
	listReq := httptest.NewRequest(http.MethodGet, "/v1/cron?workspace_root=%2Ftmp%2Fworkspace-a", nil)
	...
	if !strings.Contains(listBody, jobA.ID) {
		t.Fatalf("list body = %s, want jobA id", listBody)
	}
	if strings.Contains(listBody, "Hangzhou weather") {
		t.Fatalf("list body = %s, want workspace filter to exclude jobB", listBody)
	}

	pauseReq := httptest.NewRequest(http.MethodPost, "/v1/cron/"+jobA.ID+"/pause", nil)
	pauseRec := httptest.NewRecorder()
	handler.ServeHTTP(pauseRec, pauseReq)
	if pauseRec.Code != http.StatusOK {
		t.Fatalf("pause status code = %d, want %d", pauseRec.Code, http.StatusOK)
	}
	if !strings.Contains(pauseRec.Body.String(), "\"enabled\":false") {
		t.Fatalf("pause body = %s, want enabled false", pauseRec.Body.String())
	}

	resumeReq := httptest.NewRequest(http.MethodPost, "/v1/cron/"+jobA.ID+"/resume", nil)
	resumeRec := httptest.NewRecorder()
	handler.ServeHTTP(resumeRec, resumeReq)
	if resumeRec.Code != http.StatusOK {
		t.Fatalf("resume status code = %d, want %d", resumeRec.Code, http.StatusOK)
	}
	if !strings.Contains(resumeRec.Body.String(), "\"enabled\":true") {
		t.Fatalf("resume body = %s, want enabled true", resumeRec.Body.String())
	}

	getReq := httptest.NewRequest(http.MethodGet, "/v1/cron/"+jobA.ID, nil)
	getRec := httptest.NewRecorder()
	handler.ServeHTTP(getRec, getReq)
	if getRec.Code != http.StatusOK {
		t.Fatalf("get status code = %d, want %d", getRec.Code, http.StatusOK)
	}
	if !strings.Contains(getRec.Body.String(), "\"id\":\""+jobA.ID+"\"") {
		t.Fatalf("get body = %s, want job id", getRec.Body.String())
	}

	deleteReq := httptest.NewRequest(http.MethodDelete, "/v1/cron/"+jobA.ID, nil)
	deleteRec := httptest.NewRecorder()
	handler.ServeHTTP(deleteRec, deleteReq)
	if deleteRec.Code != http.StatusNoContent {
		t.Fatalf("delete status code = %d, want %d", deleteRec.Code, http.StatusNoContent)
	}
}
...
func TestReportingOverviewEndpointReturnsReportingState(t *testing.T) {
	deps := NewTestDependencies(t)
	store := deps.Store.(*sqlite.Store)
	now := time.Date(2026, 4, 10, 14, 0, 0, 0, time.UTC)

	if err := store.UpsertChildAgentSpec(context.Background(), types.ChildAgentSpec{
	...
	if err := store.UpsertOutputContract(context.Background(), types.OutputContract{
	...
	if err := store.UpsertChildAgentResult(context.Background(), types.ChildAgentResult{
		ResultID:        "result_1",
		SessionID:       "sess_reporting",
		AgentID:         "docker-check",
		ContractID:      "ops-monitor-v1",
	...
	if err := store.UpsertDigestRecord(context.Background(), types.DigestRecord{
		DigestID:        "digest_1",
		SessionID:       "sess_reporting",
		GroupID:         "ops-daily",
		SourceResultIDs: []string{"result_1"},
	...
	if err := store.UpsertChildAgentSpec(context.Background(), types.ChildAgentSpec{
		...
		Purpose:   "Other session worker",
		Mode:      types.ChildAgentModeBackgroundWorker,
		CreatedAt: now,
		UpdatedAt: now,
	}); err != nil {
		t.Fatalf("UpsertChildAgentSpec(other) error = %v", err)
	}

	handler := NewRouter(deps)
	req := httptest.NewRequest(http.MethodGet, "/v1/reporting/overview?session_id=sess_reporting", nil)
	rec := httptest.NewRecorder()
	handler.ServeHTTP(rec, req)
	...
	var payload types.ReportingOverview
	if err := json.Unmarshal(rec.Body.Bytes(), &payload); err != nil {
		t.Fatalf("Unmarshal(response) error = %v", err)
	}
	if len(payload.ChildAgents) != 1 || payload.ChildAgents[0].AgentID != "docker-check" {
		t.Fatalf("ChildAgents = %#v, want docker-check", payload.ChildAgents)
	}
	if len(payload.OutputContracts) != 1 || payload.OutputContracts[0].ContractID != "ops-monitor-v1" {
		t.Fatalf("OutputContracts = %#v, want ops-monitor-v1", payload.OutputContracts)
	}
	if len(payload.ReportGroups) != 1 || payload.ReportGroups[0].GroupID != "ops-daily" {
		t.Fatalf("ReportGroups = %#v, want ops-daily", payload.ReportGroups)
	}
	if len(payload.ChildResults) != 1 || payload.ChildResults[0].ResultID != "result_1" {
		t.Fatalf("ChildResults = %#v, want result_1", payload.ChildResults)
	}
	if len(payload.Digests) != 1 || payload.Digests[0].DigestID != "digest_1" {
		t.Fatalf("Digests = %#v, want digest_1", payload.Digests)
	}
}
