...
func NewStreamer(client RuntimeClient, sessionID string, afterSeq int64) *Streamer {
	...
	return &Streamer{client: client, sessionID: sessionID, afterSeq: afterSeq, ctx: ctx, cancel: cancel}
}
...
func (s *Streamer) StreamCmd() tea.Cmd {
	...
	return func() tea.Msg {
		...
		go s.run(s.ctx, ch)
		return tuiStreamReadyMsg{SessionID: s.sessionID, Ch: ch, Cancel: s.cancel}
	}
}
...
func (s *Streamer) run(ctx context.Context, out chan<- tea.Msg) {
	...
	for {
		...
		events, errs, err := s.client.StreamEvents(ctx, s.sessionID, lastSeq)
		if err != nil {
...
func (s *Streamer) forwardEvents(ctx context.Context, events <-chan Event, out chan<- tea.Msg, lastSeq *int64) bool {
	var (
	...
	flushBuffer := func() bool {
		...
		return s.sendMsg(ctx, out, tuiStreamEventMsg{SessionID: s.sessionID, Event: event})
	}
	startTimer := func() {
		if timer == nil {
	...
	for {
		...
		case event, ok := <-events:
			...
			if event.Type == "assistant_delta" || event.Type == "assistant.delta" {
			...
			if !s.sendMsg(ctx, out, tuiStreamEventMsg{SessionID: s.sessionID, Event: event}) {
				return false
			}
		}
	}
