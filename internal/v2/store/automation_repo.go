...
func (r *automationRepo) Create(ctx context.Context, a contracts.Automation) error {
	_, err := r.execer().Exec(`
INSERT INTO v2_automations (id, workspace_root, title, goal, state, owner, watcher_path, watcher_cron, created_at, updated_at)
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		a.ID, a.WorkspaceRoot, a.Title, a.Goal, a.State, a.Owner, a.WatcherPath, a.WatcherCron, timeString(a.CreatedAt), timeString(a.UpdatedAt))
	return err
}
...
SELECT id, workspace_root, title, goal, state, owner, watcher_path, watcher_cron, created_at, updated_at
...
SET workspace_root = ?, title = ?, goal = ?, state = ?, owner = ?, watcher_path = ?, watcher_cron = ?, updated_at = ?
WHERE id = ?`,
		a.WorkspaceRoot, a.Title, a.Goal, a.State, a.Owner, a.WatcherPath, a.WatcherCron, timeString(a.UpdatedAt), a.ID)
	return err
}
...
func (r *automationRepo) CreateRun(ctx context.Context, run contracts.AutomationRun) error {
	_, err := r.execer().Exec(`
INSERT INTO v2_automation_runs (automation_id, dedupe_key, task_id, status, summary, created_at)
VALUES (?, ?, ?, ?, ?, ?)`,
		run.AutomationID, run.DedupeKey, run.TaskID, run.Status, run.Summary, timeString(run.CreatedAt))
	return err
}
...
SELECT automation_id, dedupe_key, task_id, status, summary, created_at
...
func scanAutomation(row interface {
	...
	err := row.Scan(&a.ID, &a.WorkspaceRoot, &a.Title, &a.Goal, &a.State, &a.Owner, &a.WatcherPath, &a.WatcherCron, &createdAt, &updatedAt)
	if err != nil {
...
func scanAutomationRun(row interface {
	...
	err := row.Scan(&run.AutomationID, &run.DedupeKey, &run.TaskID, &run.Status, &run.Summary, &createdAt)
	if err != nil {
