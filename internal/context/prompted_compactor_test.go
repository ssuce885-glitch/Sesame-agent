...
func TestPromptedCompactorBuildsStructuredSummary(t *testing.T) {
	...
	if len(req.Items) != len(items)+1 {
		t.Fatalf("len(LastRequest().Items) = %d, want %d", len(req.Items), len(items)+1)
	}
	last := req.Items[len(req.Items)-1]
	if last.Kind != model.ConversationItemUserMessage || last.Text != promptedSummaryFollowupPrompt {
		t.Fatalf("last request item = %#v, want followup user prompt", last)
	}
	if req.Instructions == "" {
...
func TestPromptedCompactorFallsBackWhenModelReturnsPlainText(t *testing.T) {
	client := model.NewFakeStreaming([][]model.StreamEvent{{
		{Kind: model.StreamEventTextDelta, TextDelta: "当前工作目录是：E:\\project\\go-agent\n\n这是你的项目根目录。"},
		{Kind: model.StreamEventMessageEnd},
	}})

	compactor := NewPromptedCompactor(client, "compact-model")
	summary, err := compactor.Compact(context.Background(), []model.ConversationItem{
		model.UserMessageItem("compact this"),
	})
	if err != nil {
		t.Fatalf("Compact() error = %v, want fallback summary", err)
	}
	if summary.RangeLabel == "" {
		t.Fatal("RangeLabel = empty, want fallback label")
	}
	if got := strings.Join(summary.OpenThreads, ","); !strings.Contains(got, "当前工作目录") {
		t.Fatalf("OpenThreads = %v, want fallback raw text", summary.OpenThreads)
	}
}
