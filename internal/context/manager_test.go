...
func TestManagerBuildBacksCompactionStartToSafeToolBoundary(t *testing.T) {
	manager := NewManager(Config{
		MaxRecentItems:      2,
		MaxEstimatedTokens:  6000,
		CompactionThreshold: 8,
	...
	if len(got.RecentItems) != 3 {
		t.Fatalf("len(RecentItems) = %d, want 3 full exchange items", len(got.RecentItems))
	}
	if got.RecentItems[0].Kind != model.ConversationItemToolCall {
		t.Fatalf("RecentItems[0] = %#v, want tool call carried into tail", got.RecentItems[0])
	}
}
...
func TestManagerBuildSnapshotsWorkingSet(t *testing.T) {
	...
	if got.PromptItems[0].ToolCall.Input["query"] != "alpha" {
		t.Fatalf("PromptItems[0].ToolCall.Input[query] = %q, want alpha", got.PromptItems[0].ToolCall.Input["query"])
	}
