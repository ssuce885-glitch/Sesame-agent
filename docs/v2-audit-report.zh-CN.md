...
- `000006.yaml` 已同步到当前 `role.yaml/prompt.md` 的 `email + JSON API` 方向。
...
- 已补 `automation-standard-behavior` 和 `automation-normalizer` workspace skills，并同步到 `reddit_monitor` 的 role 配置和 v6 快照。
- watcher 脚本是 `0644` 时，V2 watcher 现在支持 `.py` 通过 `python3` 执行。

仍需确认：

- `prompt.md` 硬编码了收件人邮箱和代理地址，是否进入仓库需要人工确认。
- Reddit Monitor 仍需要真实自动化联调。

建议：

- 把邮箱、代理、外发确认策略从角色 prompt 中拆到配置或技能参数。
...
- `skills/` 已从 `.gitignore` 放出；需要把角色依赖技能文件纳入提交，保证 fresh clone 可复现。

## 提交前必须确认

1. `internal/v2/**` 和新增 Web 页面必须纳入版本控制，不能只提交 V1 删除。
2. `roles/reddit_monitor/.role-versions/000006.yaml` 已修正，需要随 role 配置提交。
3. V1 删除已确认是正式决策。
4. 旧 docs 是否在同一轮迁移到 V2，或者明确标注 V1 历史。
5. Fresh clone 是否能加载 role 所需 skills；`skills/` 已允许提交，但需要确认依赖技能文件进入版本控制。
